...
class AI:
    ...
    def __init__(self, history_len=20):
        self.convo_hist = deque(maxlen=history_len)
        self.total_tokens = 0
    ...
    def get_convo_history(self):
        return list(self.convo_hist)
    
    async def run(self, user_id, prompt):
        ...
        while self.total_tokens >= 3996:
            ...
            # If the oldest message is the special message, put it back in the deque and break the loop
            if oldest_message["content"] == self.SPECIAL_MESSAGE:
                self.convo_hist.appendleft(oldest_message)
                break
